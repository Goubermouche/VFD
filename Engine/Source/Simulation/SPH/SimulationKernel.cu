...
namespace fe {
	namespace sph {
		...
		static __device__ void CalculateBoundary(glm::vec3& position, glm::vec3& velocity)
		{
			...
			const float bounds = c_Description.BoundsSoftDistance;
			const float stiffness = c_Description.BoundsStiffness;
			const float damping0 = c_Description.BoundsDamping;
			const float damping1 = c_Description.BoundsDampingCritical;
			float acceleration;
			float difference;
...
#define  ADD_BOUNDS1()  acceleration = stiffness * difference - damping1 * glm::dot(normal, velocity);  velocity += acceleration * normal * c_Description.TimeStep;
		...
		static __global__ void IntegrateKernel(glm::vec4* newPosition, glm::vec4* oldPosition, glm::vec4* newVelocity, glm::vec4* oldVelocity) {
			...
			const float b = c_Description.BoundsHardDistance;
			const glm::vec3 worldMin = c_Description.WorldMin, wmax = c_Description.WorldMax;
		...
		static __device__ int3 CalculateGridPosition(const glm::vec4 position)
		{
			...
			int3 gridPosition;
			const glm::vec3 gridPositionFloat3 = (glm::vec3(position.x, position.y, position.z) - c_Description.WorldMin) / c_Description.CellSize;
		...
		static __device__ float CalculateCellDensity(const int3 gridPosition, uint32_t index, glm::vec4 position)
		{
			...
			for (uint16_t i = 0; i < c_Description.MaxParticlesInCellCount; i++)
			{
				...
				if (indexOther != index)
				{
					...
					{
						const float c = c_Description.SmoothingRadius - r2;
						density += c * c * c;
					}
				}
			}
		...
		static __device__ glm::vec3 CalculatePairForce(glm::vec4 relativePosition, glm::vec4 relativeVelocity, float PPAdd, float PPMultiply)
		{
			...
			const float r = max(c_Description.MinDist, length(relPos));
			...
			if (r < c_Description.Homogeneity)
			{
				...
				const float c = c_Description.Homogeneity - r;
				const float pressureTerm = c * c_Description.SpikyKern * PPAdd / r;
				const float velocityTerm = c_Description.LapKern * c_Description.Viscosity;
		...
		static __global__ void CalculateDensityKernel(float* pressure, float* density)
		{
			...
			const float newDensity = sum * c_Description.Poly6Kern * c_Description.ParticleMass;
			const float newPressure = (newDensity - c_Description.RestDensity) * c_Description.Stiffness;
