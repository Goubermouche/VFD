...
namespace BLAS {
    ...
    inline int indexAbsMax(const std::vector<float>& x) {
    ...
    inline float absMax(const std::vector<float>& x) {
        return std::fabs(x[indexAbsMax(x)]);
    }

    // saxpy (y=alpha*x+y) =======================================================

    inline void addScaled(float alpha, const std::vector<float>& x, std::vector<float>& y) {
...
namespace fe {  
    ...
    struct PCGSolver {
        ...
        bool solve(const SparseMatrix<T>& matrix, const std::vector<T>& rhs,
            ...
            float tol = toleranceFactor * residualOut;

            formPreconditioner(matrix);
            applyPreconditioner(r, z);
            float rho = BLAS::dot(z, r);
            if (rho == 0 || rho != rho) {
                iterationsOut = 0;
                return false;
            }
