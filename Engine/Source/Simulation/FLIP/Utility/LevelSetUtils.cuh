#ifndef LEVEL_SET_UTILS_CUH
#define LEVEL_SET_UTILS_CUH

namespace fe {
	struct LevelSetUtils {
		static __device__ __host__ float FractionInside(float phiLeft, float phiRight) {
		...
		static __device__ __host__  void CycleArray(float* arr, int size) {
			float t = arr[0];
		...
		static __device__ __host__ float FractionInside(float phibl, float phibr, float phitl, float phitr) {
		...
		template <class T>
		static __device__ __host__ void Swap(T& a, T& b) {
			T c(a);
			a = b;
			b = c;
		}
		...
		template<class T>
		static __device__ __host__ void Sort(T& a, T& b, T& c, T& d)
		{
			if (a > b) { Swap(a, b); }
			if (c > d) { Swap(c, d); }
			if (a > c) { Swap(a, c); }
		...
		template<class T>
		static __device__ __host__ T SortedTetFraction(T phi0, T phi1, T phi2, T phi3) {
			return phi0 * phi0 * phi0 / ((phi0 - phi1) * (phi0 - phi2) * (phi0 - phi3));
		}

		template<class T>
		static __device__ __host__ T SortedPrismFraction(T phi0, T phi1, T phi2, T phi3) {
			T a = phi0 / (phi0 - phi2);
			T b = phi0 / (phi0 - phi3);
			T c = phi1 / (phi1 - phi3);
			T d = phi1 / (phi1 - phi2);
		...
		static __device__ __host__ float VolumeFraction(float phi0, float phi1, float phi2, float phi3) {
		...
		static __device__ __host__ float VolumeFraction(
			float phi000, float phi100,
