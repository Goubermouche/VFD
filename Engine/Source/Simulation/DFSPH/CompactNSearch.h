...
namespace fe {
	...
	inline uint_fast64_t GetZValue(const HashKey& key) {
		return m3D_e_sLUT<uint_fast64_t, uint_fast32_t>(
			static_cast<uint_fast32_t>(static_cast<int64_t>(key.k[0]) - (std::numeric_limits<int>::lowest() + 1)),
	...
	inline void PointSet::SortField(T* lst) const
	{
		...
		std::vector<T> tmp(lst, lst + m_SortTable.size());
		std::transform(m_SortTable.begin(), m_SortTable.end(), stdext::unchecked_array_iterator<T*>(lst), [&](int i) {
			return tmp[i];
		});
	}
}
