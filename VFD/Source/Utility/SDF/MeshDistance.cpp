...
namespace vfd {
	...
	float MeshDistance::Distance(const glm::vec3& point, glm::vec3* closestPoint, unsigned int* closestFace, Triangle* closestEntity) const
	{
		...
		if (face < m_Mesh->GetFaceCount())
		{
			auto t = std::array<glm::vec3 const*, 3>{
				&m_Mesh->GetVertex(m_Mesh->GetFaceVertex(face, 0u)),
				&m_Mesh->GetVertex(m_Mesh->GetFaceVertex(face, 1u)),
				&m_Mesh->GetVertex(m_Mesh->GetFaceVertex(face, 2u))
			};
	...
	void MeshDistance::Callback(const unsigned int nodeIndex, const glm::vec3& point, float& distanceCandidate) const
	{
		...
		for (unsigned int i = node.begin; i < node.begin + node.n; ++i)
		{
			...
			auto t = std::array<glm::vec3 const*, 3>{
				&m_Mesh->GetVertex(m_Mesh->GetFaceVertex(f, 0u)),
				&m_Mesh->GetVertex(m_Mesh->GetFaceVertex(f, 1u)),
				&m_Mesh->GetVertex(m_Mesh->GetFaceVertex(f, 2u))
			};
	...
	bool MeshDistance::Predicate(const unsigned int nodeIndex, const Ref<MeshBoundingSphereHierarchy>& bsh, const glm::vec3& point, float& distanceCandidate) const
	{
		...
		const float distanceToCenterSquared = glm::dot((point - hullCenter), (point - hullCenter));
	...
	float MeshDistance::SignedDistance(const glm::vec3& point) const
	{
		...
		{
		case Triangle::A:
			normal = CalculateVertexNormal(m_Mesh->GetFaceVertex(closestFace, 0u));
			break;
		case Triangle::B:
			normal = CalculateVertexNormal(m_Mesh->GetFaceVertex(closestFace, 1u));
			break;
		case Triangle::C:
			normal = CalculateVertexNormal(m_Mesh->GetFaceVertex(closestFace, 2u));
			break;
		case Triangle::D:
			normal = CalculateEdgeNormal({ closestFace, 0u });
			break;
		case Triangle::E:
			normal = CalculateEdgeNormal({ closestFace, 1u });
			break;
		case Triangle::F:
			normal = CalculateEdgeNormal({ closestFace, 2u });
			break;
		case Triangle::G:
	...
	glm::vec3 MeshDistance::CalculateVertexNormal(const unsigned int vertex) const
	{
		if (m_PreCalculatedNormals) {
	...
	glm::vec3 MeshDistance::CalculateFaceNormal(const unsigned int face) const
	{
		if (m_PreCalculatedNormals) {
			return m_FaceNormals[face];
		}

		const glm::vec3& x0 = m_Mesh->GetVertex(m_Mesh->GetFaceVertex(face, 0u));
		const glm::vec3& x1 = m_Mesh->GetVertex(m_Mesh->GetFaceVertex(face, 1u));
		const glm::vec3& x2 = m_Mesh->GetVertex(m_Mesh->GetFaceVertex(face, 2u));
