...
namespace vfd
{
	...
	//__host__ __device__ unsigned int DensityMap::MultiToSingleIndex(const glm::uvec3& index) const
	//{
	//	return m_Resolution.y * m_Resolution.x * index.z + m_Resolution.x * index.y + index.x;
	//}

	//__host__ __device__ glm::uvec3 DensityMap::SingleToMultiIndex(unsigned int index) const
	//{
	...
	//__host__ __device__ BoundingBox<glm::dvec3> DensityMap::CalculateSubDomain(const glm::uvec3& index) const 
	//{
	//	const glm::dvec3 origin = m_Domain.min + (static_cast<glm::dvec3>(index) * m_CellSize);
	//	return { origin, origin + m_CellSize };
	//}

	//__host__ __device__ BoundingBox<glm::dvec3> DensityMap::CalculateSubDomain(unsigned int index) const
	//{
	//	return CalculateSubDomain(SingleToMultiIndex(index));
	//}

	//__host__ __device__ void DensityMap::ShapeFunction(double(&res)[32], const glm::dvec3& xi, glm::dvec3(&gradient)[32])
	//{
	//	const double x = xi[0];
	//	const double y = xi[1];
	//	const double z = xi[2];
	...
	//__device__ bool DensityMap::DetermineShapeFunction(unsigned int fieldID, const glm::dvec3& x, unsigned int(&cell)[32], glm::dvec3& c0, double(&N)[32], glm::dvec3(&dN)[32])
	//{
	...
	//__host__ __device__ double DensityMap::Interpolate(unsigned int fieldID, const unsigned int(&cell)[32], const double(&N)[32])
	//{
	//	double phi = 0.0;
	...
	//__host__ __device__ double DensityMap::Interpolate(unsigned int fieldID, const unsigned int(&cell)[32], const glm::dvec3& c0, const double(&N)[32], glm::dvec3& gradient, const glm::dvec3(&dN)[32])
	//{
	//	double phi = 0.0;
