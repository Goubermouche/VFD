...
namespace fe {
	...
	bool SceneHierarchyPanel::DrawTreeNode(const char* label, ImGuiID id, ImGuiTreeNodeFlags flags)
	{
		...
		//ImGui::PushClipRect(rowAreaMin, rowAreaMax, true);

		//bool held, hovered;
		//bool clicked = ImGui::ButtonBehavior(ImRect(rowAreaMin, rowAreaMax), id,
		//	&hovered, &held, ImGuiButtonFlags_AllowItemOverlap | ImGuiButtonFlags_PressedOnClickRelease);
		...
		bool hovered =  ImGui::ItemHoverable(ImRect(rowAreaMin, rowAreaMax), id);
