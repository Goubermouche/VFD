...
namespace fe {
	...
	struct HalfedgeHasher
	{
		...
		{
			unsigned int f = halfedge.GetFace();
			unsigned int e = halfedge.GetEdge();
	...
	struct HalfedgeEqualTo
	{
		...
		{
			unsigned int fa = a.GetFace();
			unsigned int ea = a.GetEdge();
			std::array<unsigned int, 2> va = { (*Faces)[fa][ea], (*Faces)[fa][(ea + 1) % 3] };

			unsigned int fb = b.GetFace();
			unsigned int eb = b.GetEdge();
	...
	EdgeMesh::EdgeMesh(const std::vector< glm::vec3>& vertices, const std::vector<unsigned int> faces)
		: m_Vertices(vertices)
	{
		m_Faces.resize(faces.size() / 3);
		std::copy_n(faces.data(), faces.size(), &(m_Faces.front().x));
	...
	unsigned int FaceIterator::GetVertex(unsigned int i) const
	{
	...
	IncidentFaceIterator::IncidentFaceIterator(unsigned int v, const EdgeMesh* mesh)
